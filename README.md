...
Для подготовки тестовых данных используется фабрика `EntityFactory`.
Фабрика собирает payload через Pydantic request-модели `EntityRequest` и `AdditionRequest`, поэтому тесты не формируют тело запроса вручную.
Через фабрику можно использовать payload по умолчанию или переопределять отдельные поля для разных сценариев, например для обновления сущности через PATCH.
